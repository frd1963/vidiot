...
| -qd | <nobr>--queuedir</nobr>           | **path** | *see explanation* || Look for requests in the *path* folder.  This can be a full path or a relative path, but must exist.  Default is the 'Downloads' folder in the current user's home directory.  
| -dd | <nobr>--downloaddir</nobr>        | **path** | *see explanation* || Download to the *path* folder.  This can be a full path or a relative path, but must exist.  Default is the 'DownloadedMusic' folder in the folder where we look for requests (see *--queuedir*).  
| -qp | <nobr>--queuepattern</nobr>       | **pattern** | <xmp>VIDIOT*.tsv</xmp> | *see explanation* | Expect requests to be files named according to the pattern "*pattern*".  This isn't interpreted as a regex, but just using the OS's filename pattern scheme.
| -e  | <nobr>--executable</nobr> | **path** | <xmp>/opt/homebrew/bin/yt-dlp</xmp> | *see explanation* | Use **path** as the path to the executable that does the downloading.  This can be a full path or a relative path, but must exist.
